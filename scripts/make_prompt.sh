...
# readonly SCRIPT_NS="xkcd_random_proc"
readonly SRC_DIR="./xkcd_random"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/xkcd_random"
readonly OUTPUT_MD_BASENAME="xkcd_random.flat.md"

# Copy directory safely
xkcd_random_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/xkcd_random
xkcd_random_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (xkcd_random.flat.md). Other code fences remain untouched.
xkcd_random_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  xkcd_random_proc::copy_to_tmp
  xkcd_random_proc::run_strip_docs
  xkcd_random_proc::confirm "Run coderoller-flatten-repo?"
  xkcd_random_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  xkcd_random_proc::clean_flat_md_trailing_double_hash
  xkcd_random_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  xkcd_random_proc::cleanup_tmp
}
